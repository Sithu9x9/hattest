...
@section('content')
{!! Form::open(['method' => 'PUT', 'route' => ['admin.stock-post.update',$stk_post->id],'id'=>'EditForm','files' => 'true']) !!}
<div class="box  box-primary">
    ...
    <div class="box-body">
        <div class="row">
            ...
            @if(count($stk_post_images) > 0)
                <div class="form-group col-xs-12">
                    <a href="{{route('admin.stock.allimage.destroy',$stk_post->id)}}" class="btn btn-danger btn-flat btnalldelete"><i class="fa fa-trash"></i> Delete All Old Images</a>
                </div>                
                <div class="col-xs-12 form-group">
                                ...
                                <div class="box box-primary">
                                    <div class="box-body">
                                        ...
                                        <div class="col-xs-4 text-center" style="padding:45px">                                            
                                            <a href="{{route('admin.stock.image.destroy', $image->id)}}" class="btn btn-xs btn-danger btn-flat btndelete"><i class="fa fa-trash" style="font-size: 25px"></i></a>                                            
                                        </div>
                                    </div><!-- /.box-body -->
                                </div><!-- /.box -->
                            </div>
                ...
                @endif
                <div class="col-xs-12 form-group">
                    {!! Form::hidden('stock_id', $stk_id) !!}
                    <div id="fine-uploader-manual-trigger"></div>
                </div>
                <div class="col-xs-12 form-group">
                    {!! Form::label('title', 'Title *', ['class' => 'control-label']) !!}
                    {!! Form::text('title', old('title',$stk_post->title), ['class' => 'form-control', 'placeholder' => '']) !!}
                    <p class="danger" id="title_error"></p>
                </div>
                <div class="col-xs-12 form-group">
                    {!! Form::label('des', 'Description ', ['class' => 'control-label']) !!}
                    {!! Form::textarea('des', old('des',$stk_post->des), ['class' => 'form-control', 'placeholder' => '']) !!}
...
<script>
    $(document).ready(function() {
        ...
        $('#fine-uploader-manual-trigger').fineUploader({
            ...
            request: {
                endpoint: "{{route('stock-update-upload-files',$stk_post->id)}}",
                customHeaders: {
        ...
        $('#trigger-upload').click(function() {
            $(".se-pre-con").show();
            var title = $("form#EditForm input[name=title]").val();
            var des = $("form#EditForm textarea[name=des]").val();


            if (title == '') {
                $("#title_error").append('Stock Post Title is required!');
                $(".se-pre-con").hide();
                return false;
            }else {
