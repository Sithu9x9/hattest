...
<div class="box box-primary">
    ...
    <div class="box-header with-border">
        <h3 class="box-title">Financial Report List</h3>
                ...
                <table class="table table-bordered table-striped {{ count($financials) > 0 ? 'datatable' : '' }}">
                    ...
                    <tbody>
                        ...
                        <tr data-entry-id="{{ $financial->id }}">
                            <td>{{$key+1}}</td>
                            <td style="text-align:left;">{{ $financial->title }}</td>
                            <td>{{ $financial->des }}</td>
                            <td>{{ $financial->created_by }}</td>
                            <td>{{ $financial->updated_by}}</td>
                            <td>
                                <a data-toggle="modal" data-target="#editModal{{$financial->id}}" class="btn btn-flat btn-xs btn-info"><i class="fa fa-edit"></i>&nbsp; @lang('global.app_edit') &nbsp;</a>
                                {!! Form::open(array(
                                'style' => 'display: inline-block;',
                                'method' => 'DELETE',
                                'onsubmit' => "return confirm('".trans("global.app_are_you_sure")."');",
                                'route' => ['admin.financial.destroy', $financial->id],'id' => 'delete')) !!}
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="col-xs-12 form-group">
                            {!! Form::label('title', 'Financial Report Title*', ['class' => 'control-label']) !!}
                            {!! Form::text('title', old('title'), ['class' => 'form-control']) !!}
                        </div>
                        <div class="col-xs-12 danger" id="title_error"></div>
                        <div class="col-xs-12 form-group">
                            {!! Form::label('des', 'Financial Report Year*', ['class' => 'control-label']) !!}
...
<!-- Edit Modal -->
@foreach ($financials as $key=>$financial)
<div class="modal fade" id="editModal{{$financial->id}}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="box   box-primary">
            <!--.box-header -->
            <div class="box-header with-border">
            ...
            <div class="box-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="col-xs-12 form-group">
                            {!! Form::label('title', 'Financial Report Title*', ['class' => 'control-label']) !!}
                            {!! Form::text('title', old('title',$financial->title), ['class' => 'form-control']) !!}
                            {!! Form::hidden('financial_id', $financial->id) !!}
                        </div>
                        <div class="col-xs-12 danger" id="title_edit_error"></div>
                        <div class="col-xs-12 form-group">
                            {!! Form::label('des', 'Financial Report Year*', ['class' => 'control-label']) !!}
                            {!! Form::text('des', old('des',$financial->des), ['class' => 'form-control']) !!}                          
                        </div>
                        <div class="col-xs-12 form-group">
                            {!! Form::label('file', 'File*', ['class' => 'control-label']) !!}
                            {!! Form::file('file', old('file'), ['class' => 'form-control']) !!}
                            {!! Form::hidden('old_file', $financial->file) !!}
                        </div>
                        <div class="col-xs-12 danger" id="file_error"></div>
                        <div class="col-xs-12 form-group">
...
<script>
    jQuery('form#addForm').on('submit', function(e) {
        ...
        $.ajax({
            type: 'POST',
            dataType: "json",
            url: "{{ route('admin.financial.store') }}",
            data: formData,
            processData: false,
            contentType: false,
    ...
    jQuery('form#editForm').on('submit', function(e) {
        ...
        $.ajax({
            type: 'POST',
            dataType: "json",
            url: "{{ route('admin.financial.update') }}",
            data: formData,
            processData: false,
            contentType: false,
            success: function(data) {
